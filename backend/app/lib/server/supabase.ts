...
export async function getAccessToken() {
  ...
  const cookieToken = store.get('warhex_access_token')?.value;
...
export async function supabaseFetch(path: string, init: RequestInit = {}, accessToken?: string) {
  ...
  return fetch(`${base}${path}`, { ...init, headers, cache: 'no-store', signal: init.signal ?? AbortSignal.timeout(15000) });
}
...
export async function requireUser() {
  const store = await cookies();
  const initialToken = store.get('warhex_access_token')?.value || (await headers()).get('authorization')?.replace(/^Bearer\s+/i, '').trim();
