...
export default function FollowersPage() {
  const { currentUser, leaderboardData } = useWarHexData();
  const followerCards = leaderboardData.map((entry) => ({ title: entry.user.username, description: `Level ${entry.user.level} • VIP ${entry.user.vipLevel} • Followers ${entry.user.followers.toLocaleString()}`, badge: entry.user.isOnline ? 'Online' : 'Offline' }));
  return (
    <FeaturePage
